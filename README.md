...
| `antiCrow.autoOperation` | boolean | `false` | 自動操作（Continue / Allow / 提案承認など）を有効にする。AntiCrow 経由のジョブ実行中のみ動作（手動操作時は自動承認されない） |
| `antiCrow.maxRetries` | number | `1` | タイムアウト時の自動リトライ回数。0でリトライ無効 |

> 💡 `autoOperation` がオンの場合、AntiCrow 経由のジョブ実行中に限り、Antigravity が表示する確認ダイアログ（Continue, Allow, Retry 等）の自動クリックとエージェント提案の自動承認が動作します。手動で Antigravity を操作している間は自動承認されません。
