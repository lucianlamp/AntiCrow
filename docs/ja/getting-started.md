...
> 💡 この拡張機能は、Antigravity の確認ダイアログ（Run / Allow / Continue 等）を自動的にクリックします。AntiCrow の **Auto Accept Enhanced**（Pro 機能）と組み合わせることで、VSCode コマンド承認・自動スクロール・セクション展開・権限ダイアログ処理・安全ガードまで含めた完全な自律実行環境を構築できます。
...
| `antiCrow.autoAcceptEnhanced` | `false` | 拡張自動承認（Pro 限定。pesosz/antigravity-auto-accept と併用） |
