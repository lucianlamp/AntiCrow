...
1. [AntiCrow とは？](#anticrow-とは)
2. [前提条件](#前提条件)
3. [セットアップ手順](#セットアップ手順)
4. [基本的な使い方](#基本的な使い方)
5. [ワークスペース連携](#ワークスペース連携)
6. [スラッシュコマンド](#スラッシュコマンド)
7. [設定項目リファレンス](#設定項目リファレンス)
8. [カスタマイズ](#カスタマイズ)
9. [コマンドパレットコマンド一覧](#コマンドパレットコマンド一覧)
...
**AntiCrow** は、Discord と Antigravity（AI コーディングアシスタント）をつなぐブリッジ拡張機能です。

スマホやPCの Discord から自然文でやりたいことを伝えるだけで、Antigravity が自動的にタスクを実行し、結果を Discord に返してくれます。

### ✨ できること

- 📱 **スマホから遠隔操作** — 外出先からでも Discord 経由で AI にタスクを依頼
- ⏰ **定期実行** — cron 式で毎日・毎週・毎時などの定期タスクを登録
- 🔄 **即時実行** — 今すぐやってほしいことをサッと依頼
- 📂 **複数ワークスペース対応** — プロジェクトごとに Discord カテゴリーで整理
- 📎 **ファイル添付** — 画像やドキュメントを添付して AI に分析させる
- 📊 **進捗通知** — 長時間タスクの進捗をリアルタイムで Discord に通知
- 🛡️ **セキュリティ** — Bot Token は暗号化保存、ユーザー ID によるアクセス制限
...
### 3️⃣ Discord サーバーのチャンネルを準備する

以下のチャンネルを作成してください（チャンネル名が重要です）:

| チャンネル名 | 用途 | 説明 |
| --- | --- | --- |
| `#agent-chat` | メイン | 自然文で依頼を送る汎用チャンネル。即時実行・定期登録どちらも自動判断 |
| `#logs` | 通知専用 | 実行結果や通知が送られるチャンネル。メッセージ入力は無視 |
| `#admin` | 管理 | Bot の状態確認やスケジュール管理 |

> 💡 **ヒント**: チャンネルは `agent-chat` という名前であればOKです。Bot がチャンネル名を見て用途を自動判断します。
...
`npm run deploy` はビルド・バンドル・パッケージ作成・Antigravity へのインストールを一括で行います。

### 5️⃣ 初回設定

1. Antigravity のコマンドパレット（`Ctrl+Shift+P`）を開く
2. **「AntiCrow: Set Bot Token」** を実行 → 控えておいた Bot Token を入力
3. **「AntiCrow: Start」** を実行（`autoStart` が有効なら自動で起動します）
4. ステータスバーに **`✓ AntiCrow`** が表示されれば接続完了 🎉
...
| `antiCrow.timezone` | string | `Asia/Tokyo` | CRON 実行のタイムゾーン |
| `antiCrow.responseTimeoutMs` | number | `300000` | Antigravity の応答待ちタイムアウト（ミリ秒）。デフォルト5分 |
| `antiCrow.autoStart` | boolean | `true` | Antigravity 起動時に自動で Bridge を開始する |
| `antiCrow.clientId` | string | `""` | Discord Application の Client ID（スラッシュコマンド登録に必要） |
| `antiCrow.workspacePaths` | object | `{}` | ワークスペース名 → フォルダパスの対応表 |
| `antiCrow.categoryArchiveDays` | number | `7` | ワークスペースカテゴリーの自動アーカイブ日数。0で無効 |
| `antiCrow.allowedUserIds` | string[] | `[]` | メッセージ処理を許可する Discord ユーザーID一覧。空は全員許可 |
| `antiCrow.maxMessageLength` | number | `6000` | 処理するメッセージの最大文字数。超過メッセージは拒否。0で無制限 |
...
Antigravity の確認ダイアログ（「Continue」「Allow」等）を自動でクリックするルールをカスタマイズできます。

設定画面（`Ctrl+,`）で `antiCrow.autoClickRules` を編集してください。
...
- `maxMessageLength` を超えるメッセージは拒否されます

### 🔴 長時間応答がない

- 設定の `responseTimeoutMs` の値を増やす（デフォルト: 5分）
...
`antiCrow.maxMessageLength` でメッセージの最大文字数を制限できます（デフォルト: 6,000文字）。悪意のある長大なメッセージによるリソース消費を防ぎます。
