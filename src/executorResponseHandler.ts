...
export async function sendSuggestionButtons(
    ...
    try {
        const row = buildSuggestionRow(suggestions, wsKey);
        if (row) {
            ...
            logDebug(`ResponseHandler: sent ${suggestions.length} suggestion buttons to channel ${notifyChannel}${wsKey ? ` (wsKey=${wsKey})` : ''}`);
        }
    } catch (e) {
...
export async function sendProcessedResponse(options: {
    ...
    /** ワークスペースキー（提案ボタンの customId に埋め込み用） */
    wsKey?: string;
}): Promise<{ cleanContent: string; suggestions: SuggestionItem[] }> {
    const { response, responsePath, plan, channelId, callbacks, wsKey } = options;
    ...
    let embedsSentCount = 0;
    for (const group of embedGroups) {
        try {
            await callbacks.sendEmbeds(group, EmbedColor.Response);
            embedsSentCount++;
        } catch (e) {
            logDebug(`ResponseHandler: embed group send failed (${embedsSentCount}/${embedGroups.length}): ${e instanceof Error ? e.message : e}`);
        }
    }
    // 全 embed 送信失敗時のフォールバック: プレーンテキストとして最初の2000文字を送信
    if (embedsSentCount === 0 && embedGroups.length > 0) {
        logDebug('ResponseHandler: all embed groups failed, attempting plaintext fallback');
        try {
            const fallbackText = `⚠️ ${t('response.embedFallback')}\n${normalized.substring(0, 1900)}`;
            await callbacks.sendToChannel(channelId, fallbackText, EmbedColor.Warning);
        } catch (fallbackErr) {
            logDebug(`ResponseHandler: plaintext fallback also failed: ${fallbackErr instanceof Error ? fallbackErr.message : fallbackErr}`);
        }
    }
