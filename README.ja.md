...
- � **メモリー** — 過去の学びや教訓を自動記録・活用（グローバル / ワークスペース別）
...
詳細は [セキュリティポリシー](docs/ja/security.md) と [プライバシーポリシー](docs/ja/privacy.md) をご覧ください。
...
`~/.anticrow/SOUL.md` に AI への指示を書くと、すべてのプロンプトに自動適用されます。

#### 設定ファイルの場所

```
Windows: C:\Users\<ユーザー名>\.anticrow\SOUL.md
```
...
AntiCrow は過去のタスクから学んだ教訓や重要な発見を **自動的に記憶** し、次のタスクに活かします。
...
> 💡 `MEMORY.md` は平文の Markdown です。いつでも確認・編集・削除できます。
...
| `~/.anticrow/SOUL.md` | AI に送られるカスタム指示を自分で確認・編集 |
| `autoAccept` | 自動承認のオン/オフ切り替え（ステータスバーからもトグル可能） |
| `workspaceParentDirs` | 新規ワークスペース作成時のフォルダ配置先を指定 |
| `responseTimeoutMs` | タイムアウト時間の設定 |

> 💡 `SOUL.md` は平文の Markdown ファイルです。AI に送信されるカスタム指示の内容をいつでも確認でき、不要なら削除するだけで無効化できます。
...
- **セキュリティに関する報告:** [セキュリティポリシー](docs/ja/security.md) をご覧ください
...
詳細は [免責事項](docs/ja/disclaimer.md) をご覧ください。
